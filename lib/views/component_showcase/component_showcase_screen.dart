...
class _ComponentShowcaseScreenState extends State<ComponentShowcaseScreen> {
  ...
  static const _tableRows = [
    ...
    ('#2040', 'Ravi Sharma', 'Monitor', RequestStatusDemo.pendingManager, false),
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              _section('Icon buttons & text links', _buildIconButtonsAndLinks()),
              ...
              _section('Toggle, segmented control & filter chips', _buildToggleAndSegments()),
  ...
  Widget _buildButtons() {
    return Wrap(
      ...
      children: [
        ...
        AppButton(label: 'Success', variant: AppButtonVariant.success, onPressed: () {}),
        AppButton(
        ...
        const AppButton(label: 'Small', size: AppButtonSize.small, onPressed: null),
        PillActionButton(label: 'Assign', onPressed: () {}),
        PillActionButton(label: 'Reject', variant: AppButtonVariant.destructive, onPressed: () {}),
      ],
    );
  }
  ...
  Widget _buildIconButtonsAndLinks() {
    return Wrap(
      ...
      children: [
        ...
        AppIconButton(icon: Icons.close, variant: AppButtonVariant.destructive, onPressed: () {}),
        AppIconButton(
          icon: Icons.check,
        ...
        AppTextLink(label: 'View all', trailingIcon: Icons.arrow_forward, onPressed: () {}),
      ],
    );
  }
  ...
  Widget _buildBadges() {
    return const Wrap(
      ...
      children: [
        ...
        StatusPill(semantic: AppSemantic.success, label: 'Assigned', dense: true),
        SizedBox(width: 8),
  ...
  Widget _buildIconBoxes() {
    return const Wrap(
      ...
      children: [
        ...
        IconBox(icon: Icons.monitor_outlined, semantic: AppSemantic.warning, size: IconBoxSize.lg),
  ...
  Widget _buildProgressBar() {
    return const Column(
      ...
      children: [
        AppProgressBar(label: 'Assigned', value: 0.73, valueLabel: '611', semantic: AppSemantic.brand),
        SizedBox(height: 12),
        AppProgressBar(label: 'Under repair', value: 0.09, valueLabel: '34', semantic: AppSemantic.danger),
      ],
    );
  }
  ...
  Widget _buildTextFields() {
    return const Column(
      ...
      children: [
        ...
        SizedBox(height: 14),
        AppTextField(label: 'Email (error state)', errorText: 'Incorrect email or password.'),
        SizedBox(height: 14),
        AppTextField.multiline(label: 'Note (optional)', hintText: 'Current laptop battery is failing…'),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        SegmentedControl<String>(
          ...
          options: const [
            SegmentOption(value: 'low', label: 'Low', selectedSemantic: AppSemantic.neutral),
            SegmentOption(value: 'medium', label: 'Medium', selectedSemantic: AppSemantic.brand),
            SegmentOption(value: 'high', label: 'High', selectedSemantic: AppSemantic.danger),
          ],
        ),
        const SizedBox(height: 16),
        SegmentedControl<String>(
    ...
    return Column(
      ...
      children: [
        ListItemCard(
          ...
          subtitle: 'Laptop',
          trailing: const StatusPill(semantic: AppSemantic.success, label: 'Assigned', dense: true),
          meta: const MetaEntry(icon: Icons.calendar_today_outlined, label: '04 Feb – 04 Aug 2026'),
          onTap: () {},
        ),
        ...
        const InfoRowsCard(
          rows: [
            ...
            InfoRow(label: 'Setup', value: StatusPill(semantic: AppSemantic.brand, label: 'Work from home', dense: true)),
          ],
        ),
        const SizedBox(height: 12),
        RequestCard(
          ...
          title: 'Arjun Mehta',
          statusBadge: const PriorityTag(semantic: AppSemantic.danger, label: 'High'),
          metaLine: '15 Jul – 15 Jan 2026',
          note: "Current laptop battery is failing before client demo…",
          actions: [
            AppButton(label: 'Reject', variant: AppButtonVariant.destructive, onPressed: () {}),
            AppButton(label: 'Approve', variant: AppButtonVariant.success, onPressed: () {}),
          ],
        ),
        const SizedBox(height: 12),
        SuggestionCard(
          ...
          subtitle: 'Same M-series performance, lighter and available immediately.',
          recommended: true,
          onSelect: () {},
        ),
  ...
  Widget _buildStatTiles() {
    return const StatTileRow(
      tiles: [
        ...
        StatTile(label: 'Assigned', value: '611', semantic: AppSemantic.success),
        StatTile(label: 'Pending requests', value: '28', semantic: AppSemantic.warning),
        StatTile(label: 'Open support', value: '14', semantic: AppSemantic.danger),
      ],
    );
  }
  ...
  Widget _buildAlerts() {
    return const Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        InlineAlert(semantic: AppSemantic.brand, message: 'Laptops need manager approval before reaching IT.'),
        SizedBox(height: 10),
        InlineAlert(semantic: AppSemantic.warning, message: 'Overlaps an existing booking in Nov.'),
        SizedBox(height: 10),
        InlineAlert(semantic: AppSemantic.success, message: 'The device enters return shipping and stays assigned to you until IT confirms receipt.'),
        SizedBox(height: 10),
        InlineAlert(semantic: AppSemantic.danger, message: 'Deactivation is blocked when a user has active devices or open requests.'),
      ],
    );
  }
    ...
    return Wrap(
      ...
      children: [
        AppButton(
          ...
          onPressed: () => AppToast.success(context, 'Request submitted successfully.'),
        ),
        AppButton(
          label: 'Show error toast',
          size: AppButtonSize.small,
          variant: AppButtonVariant.destructive,
          onPressed: () => AppToast.error(context, 'Something went wrong. Try again.'),
        ),
        AppButton(
          label: 'Show info toast',
          size: AppButtonSize.small,
          variant: AppButtonVariant.secondary,
          onPressed: () => AppToast.info(context, 'Your request is pending manager approval.'),
        ),
        AppButton(
          label: 'Show warning toast',
          size: AppButtonSize.small,
          variant: AppButtonVariant.secondary,
          onPressed: () => AppToast.warning(context, 'This overlaps an existing booking.'),
        ),
      ],
    );
  }
    ...
    return AppDataTable<(String, String, String, RequestStatusDemo, bool)>(
      ...
      columns: [
        TableColumn(
          header: 'ID',
          width: 70,
          cellBuilder: (context, row) => Text(row.$1, style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600)),
        ),
        TableColumn(header: 'Employee', flex: 2, cellBuilder: (context, row) => Text(row.$2)),
        TableColumn(header: 'Category', cellBuilder: (context, row) => Text(row.$3)),
        TableColumn(
          header: 'Status',
          cellBuilder: (context, row) => switch (row.$4) {
            RequestStatusDemo.pendingIt => const StatusPill(semantic: AppSemantic.warning, label: 'Pending IT', dense: true),
            RequestStatusDemo.pendingManager => const StatusPill(semantic: AppSemantic.neutral, label: 'Pending RM', dense: true),
            RequestStatusDemo.assigned => const StatusPill(semantic: AppSemantic.success, label: 'Assigned', dense: true),
            RequestStatusDemo.completed => const StatusPill(semantic: AppSemantic.neutral, label: 'Completed', dense: true),
            RequestStatusDemo.rejected => const StatusPill(semantic: AppSemantic.danger, label: 'Rejected', dense: true),
          },
        ),
        TableColumn(
          header: 'Actions',
          width: 90,
          cellBuilder: (context, row) => PillActionButton(label: 'View', variant: AppButtonVariant.secondary, onPressed: () {}),
        ),
      ],
    );
  }
  ...
  Widget _buildStaticTable() {
    return AppDataTable<(String, String)>(
      ...
      columns: [
        TableColumn(header: 'Device', flex: 2, cellBuilder: (context, row) => Text(row.$1)),
        TableColumn(header: 'Category', cellBuilder: (context, row) => Text(row.$2)),
      ],
  ...
  Widget _buildQrDisplays() {
    Widget qrPattern() => Container(color: AppColors.primaryDeep.withValues(alpha: 0.08));
    return Wrap(
      spacing: 24,
      runSpacing: 24,
      crossAxisAlignment: WrapCrossAlignment.start,
    ...
    return const VerticalTimeline(
      entries: [
        TimelineEntry(
          ...
          description: 'Actor: IT Admin · from available → assigned · request #2018',
        ),
        TimelineEntry(
...
enum RequestStatusDemo { pendingIt, pendingManager, assigned, completed, rejected }
